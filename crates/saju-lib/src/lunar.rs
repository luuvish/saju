use chrono::{Datelike, Duration, NaiveDate};

use crate::types::LunarDate;

const LUNAR_MIN_YEAR: i32 = 1900;
const LUNAR_MAX_YEAR: i32 = 2099;
const LUNAR_INFO: [u32; 200] = [
    // 1900-1909
    0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0,
    0x055d2,
    // 1910-1919
    0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2,
    0x095b0, 0x14977,
    // 1920-1929
    0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570,
    0x052f2, 0x04970,
    // 1930-1939
    0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0,
    0x1c8d7, 0x0c950,
    // 1940-1949
    0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2,
    0x0a950, 0x0b557,
    // 1950-1959
    0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8,
    0x0e950, 0x06aa0,
    // 1960-1969
    0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950,
    0x05b57, 0x056a0,
    // 1970-1979
    0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540,
    0x0b5a0, 0x195a6,
    // 1980-1989
    0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46,
    0x0ab60, 0x09570,
    // 1990-1999
    0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60,
    0x096d5, 0x092e0,
    // 2000-2009
    0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0,
    0x092d0, 0x0cab5,
    // 2010-2019
    0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176,
    0x052b0, 0x0a930,
    // 2020-2029
    0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260,
    0x0ea65, 0x0d530,
    // 2030-2039
    0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250,
    0x0d520, 0x0dd45,
    // 2040-2049
    0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255,
    0x06d20, 0x0ada0,
    // 2050-2059
    0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06aa0,
    0x1a6c4, 0x0aae0,
    // 2060-2069
    0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0,
    0x0a6d0, 0x055d4,
    // 2070-2079
    0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4,
    0x0a5b0, 0x052b0,
    // 2080-2089
    0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570,
    0x054e4, 0x0d160,
    // 2090-2099
    0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0,
    0x0d150, 0x0f252,
];

pub fn lunar_to_solar(year: i32, month: u32, day: u32, is_leap: bool) -> Result<NaiveDate, String> {
    if year < LUNAR_MIN_YEAR || year > LUNAR_MAX_YEAR {
        return Err(format!(
            "lunar date range supported: {}-{}",
            LUNAR_MIN_YEAR, LUNAR_MAX_YEAR
        ));
    }
    if month == 0 || month > 12 {
        return Err("lunar month must be 1-12".to_string());
    }
    let leap_month = lunar_leap_month(year);
    if is_leap && leap_month != month {
        return Err("leap-month flag does not match the year's leap month".to_string());
    }
    let max_day = if is_leap {
        lunar_leap_days(year)
    } else {
        lunar_month_days(year, month)
    };
    if day == 0 || day > max_day {
        return Err(format!("lunar day must be 1-{}", max_day));
    }

    let mut offset: i64 = 0;
    for y in LUNAR_MIN_YEAR..year {
        offset += lunar_year_days(y) as i64;
    }
    for m in 1..month {
        offset += lunar_month_days(year, m) as i64;
        if leap_month == m {
            offset += lunar_leap_days(year) as i64;
        }
    }
    if is_leap {
        offset += lunar_month_days(year, month) as i64;
    }
    offset += (day - 1) as i64;

    let base = NaiveDate::from_ymd_opt(LUNAR_MIN_YEAR, 1, 31).unwrap();
    base.checked_add_signed(Duration::days(offset))
        .ok_or_else(|| "converted date out of supported range".to_string())
}

pub fn solar_to_lunar(date: NaiveDate) -> Result<LunarDate, String> {
    let base = NaiveDate::from_ymd_opt(LUNAR_MIN_YEAR, 1, 31).unwrap();
    if date < base {
        return Err("solar date before supported lunar range".to_string());
    }
    if date.year() > LUNAR_MAX_YEAR {
        return Err(format!(
            "lunar date range supported: {}-{}",
            LUNAR_MIN_YEAR, LUNAR_MAX_YEAR
        ));
    }

    let mut offset = date.signed_duration_since(base).num_days();
    let total_days: i64 = (LUNAR_MIN_YEAR..=LUNAR_MAX_YEAR)
        .map(|y| lunar_year_days(y) as i64)
        .sum();
    if offset >= total_days {
        return Err("solar date after supported lunar range".to_string());
    }

    let mut year = LUNAR_MIN_YEAR;
    for y in LUNAR_MIN_YEAR..=LUNAR_MAX_YEAR {
        let days = lunar_year_days(y) as i64;
        if offset < days {
            year = y;
            break;
        }
        offset -= days;
    }

    let leap = lunar_leap_month(year);
    let mut month = 1;
    let mut is_leap = false;
    'outer: for m in 1..=12 {
        let mdays = lunar_month_days(year, m) as i64;
        if offset < mdays {
            month = m;
            is_leap = false;
            break 'outer;
        }
        offset -= mdays;

        if leap == m {
            let ldays = lunar_leap_days(year) as i64;
            if offset < ldays {
                month = m;
                is_leap = true;
                break 'outer;
            }
            offset -= ldays;
        }
    }

    let day = (offset + 1) as u32;
    Ok(LunarDate {
        year,
        month,
        day,
        is_leap,
    })
}

fn lunar_info(year: i32) -> u32 {
    LUNAR_INFO[(year - LUNAR_MIN_YEAR) as usize]
}

fn lunar_leap_month(year: i32) -> u32 {
    lunar_info(year) & 0xF
}

fn lunar_leap_days(year: i32) -> u32 {
    if lunar_leap_month(year) == 0 {
        0
    } else if lunar_info(year) & 0x10000 != 0 {
        30
    } else {
        29
    }
}

fn lunar_month_days(year: i32, month: u32) -> u32 {
    if lunar_info(year) & (0x10000 >> month) != 0 {
        30
    } else {
        29
    }
}

fn lunar_year_days(year: i32) -> u32 {
    let info = lunar_info(year);
    let mut sum = 348;
    let mut mask = 0x8000;
    while mask > 0x8 {
        if info & mask != 0 {
            sum += 1;
        }
        mask >>= 1;
    }
    sum + lunar_leap_days(year)
}
